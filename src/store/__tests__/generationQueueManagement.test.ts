...
describe('retryJob', () => {
  ...
  it('creates a new job from a failed job with retryParams', () => {
    const retryParams = {
      type: 'text2music' as const,
      prompt: 'upbeat pop',
      lyrics: '[verse]\nHello',
    };
    ...
    useGenerationStore.getState().addJob(failedJob);

    const newJobId = useGenerationStore.getState().retryJob('job-fail');
    ...
    expect(newJob?.clipId).toBe('clip-1');
    expect(newJob?.trackName).toBe('Test Track');
    expect(newJob?.retryParams).toEqual(retryParams);
  });

  it('returns null for a non-existent job', () => {
    const result = useGenerationStore.getState().retryJob('non-existent');
    expect(result).toBeNull();
  });

  it('returns null for an active job (cannot retry active jobs)', () => {
    const job = createTestJob({ id: 'job-active', status: 'generating' });
    useGenerationStore.getState().addJob(job);
    const result = useGenerationStore.getState().retryJob('job-active');
    expect(result).toBeNull();
  });

  it('returns null for a job without retryParams', () => {
    const job = createTestJob({ id: 'job-no-params', status: 'error' });
    useGenerationStore.getState().addJob(job);
    const result = useGenerationStore.getState().retryJob('job-no-params');
    expect(result).toBeNull();
  });

  it('can retry a cancelled job', () => {
    const retryParams = { type: 'text2music' as const, prompt: 'test' };
    ...
    useGenerationStore.getState().addJob(job);
    const newJobId = useGenerationStore.getState().retryJob('job-cancelled');
    expect(newJobId).toBeTruthy();
  });

  it('new retry job has a different ID from original', () => {
    const retryParams = { type: 'text2music' as const, prompt: 'test' };
    const job = createTestJob({ id: 'job-orig', status: 'error', retryParams });
    useGenerationStore.getState().addJob(job);
    const newJobId = useGenerationStore.getState().retryJob('job-orig');
    expect(newJobId).not.toBe('job-orig');
  });
});
